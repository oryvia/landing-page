        ...
        return (
            <div key={label} className="relative w-full">
                <button
                    ...
                    className={`flex items-center justify-between w-full text-sm transition-colors rounded-3xl ${hasMounted && subLinks.some(sub => sub.href === currentPath)
                            ? 'font-medium text-dark'
                            : 'text-dark'
                        } ${isMobile ? 'nav-link px-3 py-2 hover:bg-accent hover:text-white' : 'hover:text-accent'}`}
                >
                    {label}
                    <ChevronDown
                ...
                <div
                    className={`transition-all duration-300 ease-in-out transform origin-top overflow-hidden ${isMobile
                            ? 'pl-4 space-y-2'
    ...
    return (
        ...
        >
            ...
            <div className="lg:hidden">
                ...
                >
                    <Menu
                        className={`absolute inset-0 w-6 h-6 transition-transform duration-300 ${menuOpen ? 'rotate-90 opacity-0' : 'rotate-0 opacity-100'
                            }`}
                    />
                    <X
                        className={`absolute inset-0 w-6 h-6 transition-transform duration-300 ${menuOpen ? 'rotate-0 opacity-100' : '-rotate-90 opacity-0'
                            }`}
                    />
                </button>
            </div>
            ...
            <div
                className={`transition-all duration-300 ease-in-out transform origin-top lg:hidden absolute top-20 left-4 right-4 bg-white shadow-lg rounded-3xl p-5 flex flex-col items-start space-y-4 z-[100] ${menuOpen ? 'translate-y-0 opacity-100 scale-y-100' : 'translate-y-[-10px] opacity-0 scale-y-95 pointer-events-none'
                    }`}
